...
public class HTMLParser {
    ...
    Optional<Node> parseElement() {
        ...
        {
            // assuming this is <!Doctype html> so just ignore element
            consumeWhile(c -> !c.equals('>'));
            assertChar('>'); // consumes final comment bracket
            return Optional.empty();
        }else if(startsWith("head"))
        {
            //ignore children, and display none for head right now
            // consume until closing head tag
            while(!startsWith("/head"))
            {
                consumeChar();
            }
            consumeWhile(c -> !c.equals('>'));
